...
class LLM:
    ...
    def generate(self, input_text, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mode='vanilla', verbose=True, remove_stop_words=False, return_attentions=False, guiding_classifier=None, chunk_size=None, num_candidates=None, conversion_matrix=None, extra_prompt_length=None, teacher_forcing_seq=None, return_activations_layers=None, **kwargs):
        with torch.no_grad():
            ...
            output_hidden_states = return_activations_layers is not None and len(return_activations_layers) > 0
            ...
            if mode == 'vanilla':
                outputs = self.model.generate(inputs=input_ids, max_length=max_len, num_return_sequences=1,
                                    ...
                                    output_attentions=return_attentions, teacher_forcing_seq=teacher_forcing_seq, output_hidden_states=output_hidden_states, **kwargs)
                ...
                outputs = self.model.generate(input_ids, max_length=max_len, num_return_sequences=1,
                                        ...
                                        num_candidates=num_candidates, conversion_matrix=conversion_matrix, output_hidden_states=output_hidden_states, **kwargs,)
            sequences, gen_scores = outputs.sequences, outputs.scores
            scores = get_atps(self.model, input_ids, sequences, gen_scores, outputs.attentions[-1][0][0,0,:,:].shape[-1])
        ...
        if not return_attentions:
            return output_str, None, None, gen_arr, scores
        else:
            if not output_hidden_states:
                return output_str, outputs.attentions, None, gen_arr, scores
            else:
                return output_str, outputs.attentions, outputs.hidden_states, gen_arr, scores
